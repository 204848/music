...
Player.prototype.loadLyrics = function(lrcPath) {
    ...
    lyricsRequest.onload = function() {
        if (lyricsRequest.status === 200) {
            self.parseLyrics(lyricsRequest.responseText);
            console.log("歌词加载成功:", lrcPath);
        } else {
            console.error("歌词加载失败:", lyricsRequest.status, lrcPath);
        }
    };
    lyricsRequest.onerror = function() {
        console.error("歌词请求失败:", lrcPath);
    };
    lyricsRequest.send();
};

// 解析歌词（支持SRT和LRC格式）
Player.prototype.parseLyrics = function(text) {
    this.currentLyrics = [];
    ...
    // 尝试解析LRC格式
        ...
        lines.forEach(line => {
            ...
            // 匹配时间标签 [mm:ss.xx]
            let timeTags = line.match(/\[(\d{2}):(\d{2})\.(\d{2,3})\]/g);
            if (!timeTags) return;
            ...
            timeTags.forEach(tag => {
                let match = tag.match(/\[(\d{2}):(\d{2})\.(\d{2,3})\]/);
                if (match) {
                    let minutes = parseInt(match[1]);
                    let seconds = parseInt(match[2]);
                    let milliseconds = parseInt(match[3].length === 2 ? match[3] + '0' : match[3]);
    ...
    if (this.currentLyrics.length === 0) {
        console.warn("未解析到歌词内容");
        lyricsText.innerHTML = "<div class='lyric-line'>未找到歌词</div>";
        return;
    }
    ...
    // 设置歌词容器高度
    lyricsContainer.style.maxHeight = (window.innerHeight * 0.3) + 'px';
};
...
Player.prototype = {
    ...
    play: function(index) {
        ...
        if (data.lrc) {
            ...
            lyricsText.innerHTML = '<div class="lyric-line">无歌词</div>';
            this.currentLyrics = [];
        }
    ...
    step: function() {
        ...
        // 更新歌词
        if (self.currentLyrics.length > 0) {
...
document.addEventListener('keyup', function(event) {
    ...
    if (event.key == ' ' || event.key == "MediaPlayPause") {
        ...
        player.toggleVolume();
    } else if (event.key == "y" || event.key === "Y") { // 添加歌词快捷键
        player.toggleLyrics();
    }
});

console.log("\n %c Gmemp v3.5.0 %c https://github.com/Meekdai/Gmemp \n", "color: #fff; background-image: linear-gradient(90deg, rgb(47, 172, 178) 0%, rgb(45, 190, 96) 100%); padding:5px 1px;", "background-image: linear-gradient(90deg, rgb(45, 190, 96) 0%, rgb(255, 255, 255) 100%); padding:5px 0;");
